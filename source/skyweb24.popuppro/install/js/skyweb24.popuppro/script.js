var managerPopupPro={
	...
	updatePreview:function(newSrc){
		...
		for(i=0; i<currentTmplt.length; i++){
			if(currentTmplt[i]['active']){
				managerPopupPro.detailEditContentBlock.find('input[type=text], input[type=hidden], textarea, select').each(function(){
	...
	setPreviewBlock:function(activeI){
		...
		if(!managerPopupPro.detailTemplateArea){
			managerPopupPro.detailTemplateArea=$('#detail_template_area');
		}
		...
		managerPopupPro.detailTemplateArea.fadeOut(200, function(){
			...
			//managerPopupPro.detailTemplateArea.html(areaHTML);
			managerPopupPro.detailTemplateArea[0].innerHTML=areaHTML;
			managerPopupPro.detailTemplateArea.find('input, button, textarea').prop('disabled', true);
			managerPopupPro.updatePreview();
			managerPopupPro.detailTemplateArea.fadeIn('slow');
	...
	createTemplateForm:function(type){
		...
		for(i=0; i<currentTmplt.length; i++){
			...
			if(currentTmplt[i]['active']){
				...
				managerPopupPro.detailEditContentBlock.find('input[type=text], textarea, select').on('keyup click change', function(){managerPopupPro.updatePreview();});
