...
class popupEdit extends popup{
	...
	private function getPostTemplate(){
		...
		$rsET = CEventType::GetByID("FLY_POPUP_SENDER", LANGUAGE_ID);
		if(!$arET = $rsET->Fetch()){
			$et = new CEventType;
			$et->Add(array(
				"LID" => 'ru',
				"EVENT_NAME" => "FLY_POPUP_SENDER",
				"NAME" => GetMessage("fly.popup_EVENT_TYPE_NAME"),
				"DESCRIPTION" => 
					'#EMAIL# - mail
					#NAME# - name
					#PHONE# - phone
		...
		if(empty($tmpSettings['row']['cont_posttemplate'])){
			...
			$tmpSettings['row']['cont_posttemplate']=$emess->Add(array(
				...
				'SUBJECT'=>GetMessage("fly.popup_POST_SEND").' #NAME_FORM#',
				'BODY_TYPE'=>'text',
				'MESSAGE'=>'
#EMAIL_TITLE# - #EMAIL#
#NAME_TITLE# - #NAME# 
...
'
	...
	public function editPopup($request){		
		...
		foreach($viewStructure as $keyNextViewEl=>$nextViewEl){
			...
			if($keyNextViewEl=='EMAIL_TEMPLATE'){
				if($request->getPost('id_popup')=='new'){
					...
					$template['MESSAGE']=GetMessage("fly.popup_TEMPLATE_MESSAGE");
			...
			if($keyNextViewEl=='MAIL_TEMPLATE'){
				if($request->getPost('id_popup')=='new'){					
					...
					$template['BODY_TYPE']='html';
					$template['MESSAGE']=GetMessage("fly.popup_TEMPLATE_MESSAGE_ROULETTE");
					$tmpPost=$obTemplate->Add($template);
				}
			}
			if($keyNextViewEl=='EMAIL_TEMPLATE_D'){
				if($request->getPost('id_popup')=='new'){					
					$keyPost='FLY_POPUP_DISCOUNT_SEND';
					$rsET = CEventType::GetList(array('EVENT_NAME'=>$keyPost));
					if(!$arET = $rsET->Fetch()){
						$obEventType = new CEventType;
						$obEventType->Add(array(
							'LID'=>'ru',
							'EVENT_NAME'=>$keyPost,
							"NAME"=>GetMessage("fly.popup_EVENT_NAME_DISCOUNT"),
							"DESCRIPTION"=>str_ireplace("\t", '',GetMessage("fly.popup_EVENT_DESCRIPTION_DISCOUNT"))
						));
					}	
					$obTemplate = new CEventMessage;
					$template['LID']=array();
					...
					$template['EMAIL_FROM']='#DEFAULT_EMAIL_FROM#';
					$template['EMAIL_TO']='#EMAIL#';
					$template['SUBJECT']=GetMessage("fly.popup_TEMPLATE_SUBJECT_DISCOUNT");
					$template['BODY_TYPE']='html';
					$template['MESSAGE']=GetMessage("fly.popup_TEMPLATE_MESSAGE_DISCOUNT");
					$tmpPost=$obTemplate->Add($template);
				}
			}
		...
		$serviceName=(!empty($serviceName))?$serviceName:GetMessage("fly.popup_TABCOND_SERVICE_NAME").'_'.$this->idPopup;
	...
	public function setTemplate($type, $template, $newname){
		...
		$oldPath='/bitrix/components/fly/popup.pro/templates/'.$type.'_'.$template;
		$newPath='/bitrix/components/fly/popup.pro/templates/'.$type.'_custom_';
	...
	public function setColorTheme($type, $template, $colorstyle, $newname){
		...
		$oldPath='/bitrix/components/fly/popup.pro/templates/'.$type.'_'.$template.'/themes/'.$colorstyle.'.css';
		if(file_exists($_SERVER["DOCUMENT_ROOT"].$oldPath)){
			global $DB;
		...
		/*$retArr=array(
			'code'=>"custom_1",
			'newPath'=>"/bitrix/components/fly/popup.pro/templates/contact_type4/themes/custom_1.css",
			'status'=>"success",
			'id'=>2
		);*/
		return $retArr;
