...
class FlyPopup extends \CBitrixComponent{
	...
	public function executeComponent(){
		...
		$this->arResult=array(
			...
			'CONTENT'=>'',
			'LINK_TEXT'=>'',
			'LINK_HREF'=>'http://fly.ru',
			'IMG_1_SRC'=>'/bitrix/themes/.default/fly.popup/preload/gift_1.jpg',
			'IMG_2_SRC'=>'/bitrix/themes/.default/fly.popup/preload/gift_1.jpg',
				...
				if(count($errors)>0){
					...
					if(!empty($popupSetting['contact'])){
						...
						if(!empty($popupSetting['row']['cont_posttemplate'])){
							CEvent::Send("FLY_POPUP_SENDER", SITE_ID, $forSendDataArr, 'Y', $popupSetting['row']['cont_posttemplate']);
						}
						...
						if(!empty($popupSetting['contact']['iblock'])){
							...
							foreach($forSendDataArr as $keyData=>$nextData){
								if(!empty($tmpRes[$keyData.'_TITLE'])){
									...
									$propId=Fly\Popup\Tools::returnPropId($popupSetting['contact']['iblock'], $keyData, $tmpRes[$keyData.'_TITLE']);
					...
					$cookie = new Cookie("flyPopupFilling_".$this->arParams['ID_POPUP'], 'Y', time()+864000000);
		...
		$timer_array=array('banner','video','action','contact','html','coupon','roulette','discount');
			if(in_array($popupSetting['view']['type'],$timer_array)){
